...
window.onscroll = function () {
	if (!$("nav.open").length) {
		...
		if ($(window).width() < 425) {
			...
			const posTopImg = $(".mosaic .container .top").position();
			if (prevScrollpos < currentScrollPos &&
				currentScrollPos > 700 &&
				posTopImg.top < -101) {
				$(".mosaic .container .top").css("top", "+=5");
			}
			...
			// bot img
			const posBotImg = $(".mosaic .container .bot").position();
			console.log("currentScrollPos", currentScrollPos)
			if (prevScrollpos < currentScrollPos &&
				currentScrollPos > 1500 &&
				posBotImg.top > 636) {
				$(".mosaic .container .bot").css("top", "-=1");
			}
			else if (prevScrollpos > currentScrollPos &&
				currentScrollPos > 1500 &&
				posBotImg.top < 666) {
			...
			// phone img
			const posPhoneImg = $("  .phones .container img").position();
			if (prevScrollpos > currentScrollPos &&
				currentScrollPos > 1900 &&
				posPhoneImg.top > -105) {
				$("  .phones .container img").css("top", "-=5");
			}
			else if (prevScrollpos < currentScrollPos &&
				currentScrollPos > 1900 &&
				posPhoneImg.top < -75) {
...
function PageLoad() {
	...
	setTimeout(function () {
		...
		TweenMax.to($('.hold-progress-bar'), 0.5, {
			force3D: true, width: '0', delay: 1, ease: Power2.easeOut, onComplete: function () {  //modified time
						...
						setTimeout(function () {
							...
							if ($('#hero').hasClass("has-image")) {
								...
								TweenMax.to($(".hb-left"), 0.4, {
									force3D: true, y: 0, opacity: 1, delay: 1.1, ease: Power2.easeOut, onComplete: function () {
...
function FirstLoad() {
	...
	if ($("#page-content").hasClass("light-content")) {
		$("main, #main-content").css('background-color', '#141414');
		$('#magic-cursor').addClass('light-content');
		if ($('#hero').length > 0) {
			if ($('#hero').hasClass("has-image")) {
...
function LazyLoad() {
	...
	if ($('#hero').hasClass("has-image")) {
		if ($('body').hasClass("load-project-thumb")) {
			...
			TweenMax.to($(".hb-left"), 0.4, {
				force3D: true, y: 0, opacity: 1, delay: 0.8, ease: Power2.easeOut, onComplete: function () {
					$('.hb-left').addClass('enable');
				}
			});
			...
			TweenMax.to($(".hb-left"), 0.4, {
				force3D: true, y: 0, opacity: 1, delay: 0, ease: Power2.easeOut, onComplete: function () {
					$('.hb-left').addClass('enable');
				}
			});
		...
		var swiperOptions = {
			...
			pagination: {
				...
				renderBullet: function (index, className) {
					return '<div class="outer ' + className + '">' + '<div class="inner">' + '<div class="title">' + titles[index] + '</div>' + '<div class="subtitle">' + subtitle[index] + '</div>' + '<div class="counter-wrap">' + '<div class="counter">' + counter[index] + '</div>' + '</div>' + '</div>' + '</div>';
